...
fn main() -> anyhow::Result<()> {
    ...
    match args.cmd {
        ...
        KnnCmd::Search(arg) => knn_search(reader, arg),
    }
}
...
fn collect_records<R>(mut reader: Reader<R>, label: usize, columns: &[usize]) -> anyhow::Result<Vec<KnnRecord>>
where
    R: std::io::Read
{
...
fn split_dataset<'a>(records: &'a [KnnRecord], split: f64) -> (Vec<&'a KnnRecord>, Vec<&'a KnnRecord>) {
...
fn knn_search<R>(mut reader: Reader<R>, arg: SearchArgs) -> anyhow::Result<()>
where
    R: std::io::Read,
{
    if arg.columns.is_empty() {
